...
**Deuda conocida de la fase 10:** `plan.profile` se guarda pero no lo usa ningún cálculo (lo natural: que elija el escenario de supuestos por defecto). La edad de jubilación está fijada a 67 en Core e ignora lo que envíe el cliente. La UI aún no muestra el impacto completo de una decisión (activos/pasivos enlazados), aunque `GET /api/plan/events/{id}/budget-lines/` ya lo devuelve.
