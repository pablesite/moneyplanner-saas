...
| Frontend redesign "Direction A" — Phase 1         | Medium   | 🔄     | Patrimonio (`NetWorthView`): rework de fidelidad aplicado — hero a 2 columnas Activos/Pasivos sin KPIs extra (delta este mes + YTD), `NetWorthDonut` reescrito a SVG (hover actualiza el centro), tabla Balance con total de pasivos en negativo y sin subcategoría duplicada, charts migrados a clases del sistema y componentes heredados (`ItemList`/`SettingsPopover`/`*Workspace`/`*TimelineMain`/`*HeroSection`/`*Bar`) eliminados. Modal de cuenta remaquetado al patrón sheet Direction A (campos underline), gráfico de Evolución reescrito a SVG a medida (`NetWorthEvolutionChart`: línea suave + 8 ticks + delta integrado + hover) y vista responsive (móvil/tablet/escritorio). **Grep anti-híbrido limpio.** Pendiente: gate visual (capturas vs prototipo) + visto bueno del usuario antes de cerrar. Spec: `docs/tasks/frontend-redesign-direction-a/phase-1-networth-pilot/frontend.md` · Método: `docs/tasks/frontend-redesign-direction-a/README.md` |
| Frontend redesign "Direction A" — Phase 2         | Medium   | 🔄     | Presupuesto + Cierre mensual: presentación **desacoplada** en dos vistas independientes (`BudgetView` / `MonthlyCloseView`) sobre el motor compartido `useBudgetDashboardPage` (intacto). `/presupuesto`: hero + `AContextBar` (Año fiscal · Titularidad · Tipo · Vista · Importes) + tab Sugerencias (acciones deshabilitadas) + tabla jerárquica `bdg-row` con barra de ejecución (`BudgetBarCell`), columna Evol. (`ASparkline`), cobertura YTD, gráfico de evolución y rollup. `/cierre-mensual`: `APageHead` + `AStepper` (4 pasos) + `MonthlyCloseHero` + secciones Liquidez/Ingresos/Gastos/Resultado reskineadas (`.mc-*`, chips `AKindChip`). Primitivas nuevas: `AStepper`, `ASparkline`. Legacy retirado (`BudgetDashboardView`/`BudgetHeroSection`/`dashboard.css`). **Grep anti-híbrido limpio · 38 tests verdes.** Pendiente: gate visual (capturas vs prototipo) + visto bueno del usuario. Spec: `docs/tasks/frontend-redesign-direction-a/phase-2-budget-monthly-close/frontend.md`            |
| Frontend redesign "Direction A" — Phase 3         | Medium   | ⚪     | Movimientos: port `AccountingMovementsView` (3 tabs, filter bar, daily timeline, modals) reusing recolored charts. Spec: `docs/tasks/frontend-redesign-direction-a/phase-3-movements/frontend.md`                                                                        |
| Frontend redesign "Direction A" — Phase 4         | Medium   | 🔄     | Estado financiero (`/estado-financiero` + `/estado-financiero/ambitos/:phaseId`): port Direction A implementado con mapa/foco, strip de 5 ámbitos y grades A–E sobre scores reales, detalle a 2 columnas (puntuación + diagnóstico), selector de extraordinarios y estados loading/error/ámbito inexistente/en construcción. **Grep anti-híbrido limpio · lint/format/typecheck + 14 tests verdes.** Pendiente: gate visual happy-path con Core local disponible + visto bueno del usuario. Spec: `docs/tasks/frontend-redesign-direction-a/phase-4-guide/frontend.md` |
| PWA — Phase 1 (instalable)                        | Medium   | ⚪     | Frontend instalable como PWA: manifest, iconos, service worker de precache del shell y toast de actualización. **Sin cachear API autenticada.** Spec: `docs/tasks/pwa/phase-1-installable/frontend.md` · Método: `docs/tasks/pwa/README.md`                              |
| PWA — Phase 2 (offline)                           | Medium   | ⚪     | Resiliencia offline del shell: fallback de navegación offline, fuentes Geist auto-hospedadas y estados "sin conexión" en la UI. Depende de Phase 1. Spec: `docs/tasks/pwa/phase-2-offline/frontend.md`                                                                   |
...
| Accounting Movements | ✅ Implemented | `/movimientos`         | View closed as v1. User-facing rows expose account/origin, destination, category/subcategory and account-aware signed impact instead of raw debit/credit columns. All movement modals use the grouped/searchable Direction A selector. Multi-currency support for investment create/edit quick flows. Ad-hoc MoneyWiz importer removed; imported movement traceability preserved.                                                                                       |
