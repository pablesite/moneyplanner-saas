...
| Limpieza legacy residual Core/SaaS | Media | ⚪ | Core mantiene inventario vivo en `core/docs/roadmap/product-roadmap.md`: `data-input`, fallback Budget/check-ins, `investment_purchase`, facade `net_worth.services`, portable compatibility y `compat.*`. SaaS debe espejar las retiradas frontend cuando se ejecuten. |
