...
| Limpieza legacy residual Core/SaaS | Media | 🔄 | Core mantiene inventario vivo en `core/docs/roadmap/product-roadmap.md`. Avance espejado: Introducción de Datos absorbido en Presupuesto (`views/budget`, `budget/annual-entries`, `budget/taxonomy`) y `core.dataInput` retirado; quedan fallback Budget/check-ins, `investment_purchase`, facade `net_worth.services`, portable compatibility y `compat.*`. |
