...
| Frontend redesign "Direction A" — Phase 2         | Medium   | ⚪     | Presupuesto + Cierre mensual: port `BudgetDashboardView` (both `budget`/`monthly-close` modes — coupled, shared view + `dashboard.css`); adds `AStepper` primitive. Spec: `docs/tasks/frontend-redesign-direction-a/phase-2-budget-monthly-close/frontend.md`            |
