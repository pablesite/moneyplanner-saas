...
| 6     | Agent | ⚪     | Event lifecycle: close an incorporated plan event (asset sold/scrapped) via `POST /api/plan/events/{id}/close/` — retire its recurring budget lines from the effective date (history preserved), stop applying its deltas in the projection, and SaaS UI action with confirmation in the events timeline. Rationale: recurring scenario expenses are indefinite while the asset exists (Core commit `ff22948`). | `core/docs/tasks/financial-plan/phase-6-event-lifecycle/backend.md` + `docs/tasks/financial-plan/phase-6-event-lifecycle/frontend.md` |
| 7     | Agent | ⚪     | **Engine correctness (P0 — do first).** Browser audit with real data (`docs/tasks/financial-plan/browser-audit-2026-07-11.md`) proved the on-screen diagnosis is wrong: foundations sum budget entries across **all** fiscal years (real `committed_surplus = -78.834,58 €` vs `/presupuesto` FY2026 balance `+44.579,57 €`), the projection treats `one_off` income as perpetual labour income, `employment_income_end_date` is never applied, and expense buckets are inconsistent between projection and foundations. Fixes: single fiscal-year window helper, structural-only income, labour-income cut-off, exhaustive expense taxonomy. | `core/docs/tasks/financial-plan/phase-7-engine-correctness/backend.md` |
| 8     | Agent | ⚪     | **Budget ↔ plan boundary.** Two budget levels made explicit: recurring budget (manual entry in `/presupuesto`) vs plan-managed entries (created only from Mi Plan scenarios, `event_group=plan_event:*`). Today those lines are editable/deletable from `/presupuesto` without the plan noticing, and `event_group` is a free-text field the user can forge. Backend locks the lineage and exposes `GET /api/plan/events/{id}/budget-lines/`; frontend marks them read-only and simplifies manual entry classification. Precondition for phase 6. | `core/docs/tasks/financial-plan/phase-8-budget-plan-boundary/backend.md` + `docs/tasks/financial-plan/phase-8-budget-plan-boundary/frontend.md` |
| 9     | Agent | ⚪     | **Errors + progressive disclosure + mobile (run after phase 7).** Bug: every plan-domain API error renders as «No se pudo cargar Mi Plan.» because the store reads `data.detail` instead of the canonical `{error:{code,message,details}}`. UX: `/plan` delivers the whole diagnosis at once — headline first, detail on demand; scenarios list before form; incorporated-scenario detail must show its real budget trace; mobile fixes for `/plan/activos` (~16.000 px page), trajectory chart and plan subnav. | `docs/tasks/financial-plan/phase-9-plan-ux-3/frontend.md` |
