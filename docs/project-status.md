...
| 7     | Agent | ✅     | Engine correctness complete: one current-FY window, structural-only income, aggregate labour cut-off at the latest adult date, exhaustive expense taxonomy and explicit quality factors. Free cash now feeds both capital buckets: 25% to the emergency fund until it reaches two inflation-adjusted years of recurring expenses, and 75% to productive capital; after the cap, 100% goes to productive. Financed disposals take gross asset value and associated debt separately, then remove the net classified value so the mortgage is not subtracted twice. Real-data result: committed surplus `-78.834,58 €` → `-31.771,75 €`; projected year disappears after removing perpetual one-off income and applying the 2049 cut-off. | `core/docs/tasks/financial-plan/phase-7-engine-correctness/terminados/backend.md` |
