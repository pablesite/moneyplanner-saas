...
| 11    | Manual | ✅    | **Siembra de Presupuesto desde `/plan/setup` (solo frontend SaaS).** Un usuario sin ninguna partida de `AnnualIncomeEntry`/`AnnualExpenseEntry` real ve dos pasos nuevos ("Tus ingresos"/"Tus gastos") justo después de "Quiénes sois", antes de "Cuándo". Los importes que rellena crean partidas reales en Presupuesto (nómina/otros ingresos; una por cada subcategoría de "Gastos de consumo") reutilizando `useAnnualIncomeStore`/`useAnnualExpenseStore` — sin endpoints nuevos. Al salir del paso de gastos se refresca `foundations`, así que la sugerencia de nivel de vida del paso "Con cuánto" (antes vacía para un usuario nuevo) ya sale rellena con el gasto recién capturado. Upsert por categoría/subcategoría si el usuario vuelve atrás y reenvía el paso (evita duplicar partidas). Gating: un usuario que ya tiene alguna partida no ve estos pasos. | — |
