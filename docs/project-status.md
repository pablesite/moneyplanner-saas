...
| Frontend redesign "Direction A" — Phase 4         | Medium   | 🔄     | Guía (`/guia` + `/guia/fases/:phaseId`): port Direction A implementado con mapa/foco, strip de 5 fases y grades A–E sobre scores reales, detalle a 2 columnas (puntuación + diagnóstico), selector de extraordinarios y estados loading/error/fase inexistente/en construcción. **Grep anti-híbrido limpio · lint/format/typecheck + 14 tests verdes.** Pendiente: gate visual happy-path con Core local disponible + visto bueno del usuario. Spec: `docs/tasks/frontend-redesign-direction-a/phase-4-guide/frontend.md` |
