...
| Limpieza legacy residual Core/SaaS | Media | 🔄 | Core mantiene inventario vivo en `core/docs/roadmap/product-roadmap.md`. Avance espejado: `portable-data`, `budget/annual-entries` y `budget/taxonomy` extraídos de `data-input`; queda absorber `views/data-input/*` en Presupuesto, fallback Budget/check-ins, `investment_purchase`, facade `net_worth.services`, portable compatibility y `compat.*`. |
