...
7. In Patrimonio, Direction A should read as one continuous surface controlled by internal tabs: General, Evolución, and Balance. General contains the hero: big net-worth figure + delta (este mes / YTD) on the left, the structural SVG donut centered (hover updates its center), and two columns Activos / Pasivos on the right that cross-filter the timeline. Evolución owns the timeline controls/chart. Balance owns the grouped table on desktop and the operational compact list on mobile, with sticky search/type controls, per-position detail sheet, and thumb-reachable create action.
