...
7. In Patrimonio, Direction A should read as one continuous surface controlled by internal tabs: General, Evolución, and Balance. The header stays compact: no primary create CTA, no visible labels for the obvious read controls, no duplicated currency copy, and no permanent `Hoy` chip; show a dated base only when it adds real context. Ownership/currency/valuation controls stay in a single-row context rail that can scroll horizontally on narrow mobile. Archived positions appear as quiet actionable metadata, not as a primary action. General contains the hero: big net-worth figure on the left with tooltip-backed comparison groups for Mes/Año, each separating `vs cierre anterior` from an explicit equivalent-date label such as `vs 28 de mayo` when Core can calculate the reference date, plus capital-owned ratio as a compact chip under the figure. The Activos / Pasivos composition columns are the main explanatory element; do not reintroduce the capital-own donut or a redundant KPI strip below. Evolución owns the timeline controls/chart. Balance owns the grouped table on desktop and the operational compact list on mobile, with sticky search/type controls, per-position detail sheet, and thumb-reachable create action.
