...
20. Cartera (`/cartera`) es una profundización de Patrimonio, no un sexto destino móvil. Patrimonio permanece activo en el shell y ofrece entrada visible a Cartera incluso cuando sus acciones de cabecera se ocultan a `sm`; Cartera mantiene `Volver a Patrimonio` en su page head. La vista usa tabs sticky `Resumen`, `Posiciones` y `Evolución`, más un context rail de titularidad, contenedor, clase, divisa y periodo. El hero responde primero valor, TWR, resultado y aportaciones; la cobertura parcial se expresa con `≥ valor cubierto` y nunca fabrica una rentabilidad. Resumen continúa con un centro de atención agrupado (calidad, estructura/riesgo y ejecución) antes de composición y calidad: cada aviso explica el dato y lleva a la corrección concreta, sin decir qué comprar o vender. Posiciones usa tabla desktop y lista compacta móvil con sheet; Evolución compara valor con aportaciones y ofrece la serie en tabla accesible. Los filtros de inventario no se presentan como alcance del hero hasta que Core pueda agregarlos matemáticamente.
