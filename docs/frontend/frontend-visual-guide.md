...
7. In Patrimonio, Direction A should read as one continuous surface controlled by internal tabs: General, Evolución, and Balance. The header stays compact: no primary create CTA, no visible labels for the obvious read controls, no duplicated "Base EUR" copy, and ownership/currency/valuation controls stay in a single-row context rail that can scroll horizontally on narrow mobile. Archived positions appear as quiet actionable metadata, not as a primary action. General contains the hero: big net-worth figure + delta (este mes / YTD) on the left, the structural SVG donut centered (hover updates its center), two columns Activos / Pasivos that cross-filter the timeline, and a compact executive strip with the main composition/debt/change signals. Evolución owns the timeline controls/chart. Balance owns the grouped table on desktop and the operational compact list on mobile, with sticky search/type controls, per-position detail sheet, and thumb-reachable create action.
