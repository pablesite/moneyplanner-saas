...
Desde la Fase 9, `/plan` organiza el contenido en las pestañas URL-synced `Resumen`, `Trayectoria`, `Decisiones` y `Diagnóstico`, siguiendo el patrón de vista principal de Patrimonio. La creación de escenarios es una acción primaria visible (`Simular decisión` — etiqueta única en cabecera, laboratorio y `/plan/escenarios`) y también abre directamente desde `Decisiones`; `/plan/escenarios` prioriza la lista navegable y abre el simulador bajo demanda, con scroll automático al formulario. Una decisión admite varios desembolsos puntuales con concepto e importe; el detalle distingue su suma, financiación y gasto recurrente, y el presupuesto incorporado conserva cada concepto. Los hitos de año muestran también la edad derivada de los adultos con fecha de nacimiento. `/plan/activos` mantiene búsqueda sticky y grupos colapsables para móvil.

Tras la revisión UX de julio de 2026: el desvío frente a la fecha objetivo usa tono semántico (`pos`/`neg`, como los deltas de Patrimonio) en el hero, la tarjeta de fechas y la columna Diferencia de la comparación; en la toolbar conviven el selector `Hipótesis` (Prudente/Esperado/Favorable) y el drawer `Parámetros` (antes "Supuestos", renombrado para no chocar); `Recalcular` es acción discreta. `/plan/decisiones/registrar` pliega por defecto los años de partidas y los grupos Pasivos/Activos de Patrimonio (la búsqueda expande coincidencias) y oculta las partidas no adoptables tras un toggle con recuento.
