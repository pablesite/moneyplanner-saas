...
`PlanOneOffMovementsPlanner` (sección desplegable "Movimientos puntuales previstos" del Resumen, colapsada por defecto) resuelve la brecha de **reprogramar o reajustar** un ingreso o gasto puntual que aún no ocurre: el presupuesto es FY-scoped y el plan es el sitio natural para decidir *cuándo* y *cuánto* entra. Reutiliza `useAnnualIncomeStore('saas')` y `useAnnualExpenseStore('saas')` (mismos endpoints `PATCH /api/budget/annual-{income,expense}/{id}/`, sin backend nuevo: `fiscal_year`, `target_month` y `amount_annual` ya son escribibles); lista los movimientos `one_off` con año ≥ actual (excluye los gastos `is_system_generated` que gobierna Patrimonio; las partidas `is_plan_managed` se muestran sin edición) en una **tabla estilo Movimientos** (`Concepto · Importe · Cuándo`) con chip Ingreso/Gasto. Cada fila es una **etiqueta de solo lectura** con acción "Editar" → modo edición inline (importe + mes + año + Guardar/Cancelar/Quitar), e incluye un enlace secundario "Revisar presupuesto" (movido desde el hero). En un apretón **transitorio** (`cash_flow.committed_status === 'transient'`) el Resumen **oculta** la tarjeta "Lo más útil ahora": no hay mejora accionable — los compromisos vencen solos — y el diagnóstico del hero ya lo explica. Paridad de edición: el formulario de presupuesto (`AnnualEntryModalForm.vue`) expone Mes objetivo y Año previsto para partidas `one_off` (cableado en `useBudgetAnnualEntriesPage.ts` vía `resolveDraftFiscalYear`), de modo que importe/mes/año se editan tanto desde Mi Plan como desde Presupuesto.
