...
Desde el rediseño integral de julio de 2026, la navegación canónica es **Resumen · Mejoras · Decisiones**. `Resumen` consume `GET /api/plan/overview/`, presenta un rango prudente-favorable y una sola acción prioritaria; la trayectoria queda como detalle desplegable con alternativa tabular. `/plan/mejoras` ordena acciones por dependencia y exige preview sin efectos antes de crear un borrador. `/plan/decisiones` reúne borradores, previsiones y eventos; `/plan/escenarios*` redirige conservando query params. Los errores parciales conservan datos útiles y los cuerpos HTML se sustituyen por un mensaje seguro con referencia HTTP.
