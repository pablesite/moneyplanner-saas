...
Tras la primera revisión UX de julio de 2026: el desvío frente a la fecha objetivo usa tono semántico (`pos`/`neg`, como los deltas de Patrimonio) en el hero, la tarjeta de fechas y la columna Diferencia de la comparación; la renta sostenible también se colorea frente al objetivo mensual. El hero muestra el bloque de causas ("Qué está frenando el plan" / "Por qué no hay fecha") **también cuando existe fecha proyectada** si la salud es crítica: 0 activos cargados (CTA a Patrimonio), capital sin clasificar (CTA a `/plan/activos`) o déficit comprometido (CTA a Presupuesto). En la comparación de escenarios, al apilarse en pantallas estrechas cada valor lleva su etiqueta (Vigente/Simulado/Diferencia). Las confirmaciones de la timeline (dar de baja, materializar) se renderizan inline dentro de la fila afectada sin ocultar el resto de la lista, y el mensaje de éxito queda anclado a esa fila. Los cimientos muestran "Sin deuda" (sin tono crítico) cuando no hay pasivos. `/plan/decisiones/registrar` pliega por defecto los años de partidas y los grupos Pasivos/Activos de Patrimonio (la búsqueda expande coincidencias) y oculta las partidas no adoptables tras un toggle con recuento. `/plan/setup` ofrece salida (`Salir` → `/plan`) en todos los pasos y un hint de qué falta cuando `Continuar` está deshabilitado.
