...
19. Direction A custom period picker (`/movimientos`): cuando un filtro no puede ser `ASelect` por necesitar rango libre, debe respetar el mismo chrome base (`.filter-ctrl`, borde, radio, tipografía, espaciado y popover surface) y evitar affordances legacy como carets textuales o paneles azules ad hoc. El picker de período de Contabilidad se considera la referencia de este caso.
