...
| 4 | Guía (`HomeView` + `GuidePhaseDetailView`) | 🔄 port Direction A implementado en `/guia` + detalle: strip A–E con scores reales, panel de puntuación/diagnóstico, estados y responsive; lint/format/typecheck + 14 tests verdes. Pendiente gate visual happy-path con Core local disponible y visto bueno del usuario |
