...
| `POST` | `/api/plan/events/occurred/` | Registers a decision already taken: creates a `PlanEvent` with `status=occurred`, creates no budget rows, and adopts the existing ones passed in `expense_entry_ids`/`income_entry_ids`. Rows generated by an asset or liability, or already owned by another event, are rejected. Excluded from the projection because its effects are already in current net worth and budget. |
