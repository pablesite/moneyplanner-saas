...
| `GET` | `/api/portfolio/workspace/?container_id=&asset_class=&currency=&include_timeline=` | Overview, performance, positions, timeline and quality for one period off a single context load. An inventory filter scopes **all** of them and the payload echoes the surviving position ids in `scope`; only the positions list stays whole, since the table filters client-side over the same set. `include_timeline=false` returns the same timeline envelope with an empty series and limits the context to the requested period; it is for views that do not render Evolución. It defaults to `true` for API compatibility. |
