...
| `GET` | `/api/portfolio/allocation/?ownership_id=&on_date=` | Actual against target for one scope, by class and by position, with the band each one sits in. What you hold without having planned it shows as `unplanned` rather than hidden. |
...
| `GET/POST` | `/api/portfolio/baskets/` | Contribution baskets. `POST` solves and persists a proposal; nothing reaches accounting until confirmed. |
