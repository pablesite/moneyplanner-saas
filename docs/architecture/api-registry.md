...
| `GET` | `/api/portfolio/workspace/?container_id=&asset_class=&currency=&include_timeline=&include_position_details=` | Overview, performance, positions, timeline and quality for one period off a single context load. An inventory filter scopes **all** of them and the payload echoes the surviving position ids in `scope`; only the positions list stays whole, since the table filters client-side over the same set. `include_timeline=false` returns the same timeline envelope with an empty series and limits the context to the requested period; it is for views that do not render Evolución. `include_position_details=false` returns the value/class rows needed by Resumen without per-position performance or attribution; `position_details` declares that reduced shape. Both options default to `true` for API compatibility. |
