...
| `POST` | `/api/accounting/transactions/quick-entry/` | Now also accepts `investment_units` and `investment_unit_price` on `investment` movements. When the movement pairs a position's ledger account with the cash of that position's own container, Core leaves a `PortfolioTrade` for it — booking money is Contabilidad's job, so the operation record has to come from there too. |
| `GET/POST/PATCH/DELETE` | `/api/portfolio/strategies/?ownership_id=` | Investment policy per ownership scope, versioned by `effective_from`. Writing a strategy writes its targets in the same call; a target belongs to an asset class or to a position, never both, and `unclassified` is refused because it is the absence of an answer. Carries `max_cost_share` and `min_line_amount`, which are policy, not constants. |
...
| `GET/POST/PATCH/DELETE` | `/api/portfolio/commitments/` | Amounts that must reach a position regardless of drift: a monthly floor that keeps a broker perk, or an annual quota — which is a ceiling too, since contributing above a deductible cap does not deduct. |
| `GET` | `/api/portfolio/allocation/?ownership_id=&on_date=` | Actual against target for one scope, by class and by position, with the band each one sits in. What you hold without having planned it shows as `unplanned` rather than hidden. Linked container cash counts as liquidity, so the policy line for cash reads against real money. `suggested_contribution` carries what the budget planned to invest that month — read-only: choosing another figure does not rewrite the budget. |
...
| `POST` | `/api/portfolio/contribution/solve/` | Simulates the split of a contribution without saving anything. |
