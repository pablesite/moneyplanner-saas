...
| `GET` | `/api/portfolio/workspace/` | Overview, performance, positions, timeline and quality for one period off a single context load, plus `scoped_performance` when `container_id`/`asset_class` narrow the inventory. Replaces issuing the five reads separately. |
