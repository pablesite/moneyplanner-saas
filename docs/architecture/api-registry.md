...
| `GET` | `/api/portfolio/exposure/?on_date=` | Aggregate exposure with coverage and source per dimension, class composition, concentration, approximate overlap by declared exposure and exact overlap where dated holdings share the same identifier (or, less reliably, the same normalized name). Holdings take precedence; manual declarations remain the fallback. `classes.percent_basis=positions_total`: class percentages include the unknown fraction as `unclassified` and use the full valued positions total; geography/sector/vehicle keep their declared bases. Cash is excluded here and included in allocation, pending decision-system task 2. |
