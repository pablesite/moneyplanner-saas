...
| `GET` | `/api/portfolio/allocation/?ownership_id=&on_date=` | Actual against target for one scope, by class and by position, with the band each one sits in. `by_position` adds `class_breakdown` (class value and percentage of the allocation total), `composition_source`, `composition_observed_on` and `class_covered_percent`. Class actuals use the latest non-future holdings snapshot, then manual split, then effective class; incomplete holdings retain their remainder as `unclassified`. Product-level targets still describe the whole product. `by_position` carries every position's effective target, whether it was written or inherited from its class split by current weight (`band: derived`), so the second level is answerable without writing a line per product. Drift is positive when a slice runs ahead of its target and negative when it falls short. What you hold without having planned it shows as `unplanned` rather than hidden. Linked container cash counts as liquidity, so the policy line for cash reads against real money. `cash` and `quality` publish the dated cash perimeter and evidence blockers/warnings with corrective actions. `planned_contribution` carries what the budget planned to invest that month, `contributed_this_month` what has actually gone in **net of divestments** —selling a fund to buy another is not new money, and counting only the inflows let a reshuffle eat the month's budget without a euro leaving your pocket— and `suggested_contribution` the difference, which is what is left to contribute. All read-only: choosing another figure does not rewrite the budget. |
| `GET` | `/api/portfolio/benchmark/?ownership_id=&date_from=&date_to=` | The scope against **its own written policy**, month by month: the strategy version in force each month composes what its target weights would have returned given how the user's own classes behaved, so the excess answers whether drifting from the plan helped. Cash is out of both sides and the renormalization is declared. A month whose class has no computable return is published without benchmark instead of redistributing its weight. `benchmark_annualized_return` is the compounded annualized policy return over the longest complete run of monthly benchmark observations. `rolling` exposes equivalent 12-month windows, including only windows whose two series are complete. An index benchmark travels as optional `secondary`, with its monthly returns when available. |
| `GET` | `/api/portfolio/risk/?ownership_id=&date_from=&date_to=` | Annualized return, volatility, max drawdown, best/worst month and Sharpe on a month-end calendar, each with `available`/`insufficient` and its reason. `coverage` reports the window actually measured —the longest uninterrupted stretch— and which months are missing. `risk_free_rate` travels with the figures that use it. `advanced.value_at_risk` is historical 95% one-month VaR and requires 24 complete months; a configured, same-currency secondary index with complete closes unlocks `advanced.beta` and `advanced.correlation` after 12 common observations. `advanced.position_correlation` and `advanced.risk_contribution` use only positions with a full common monthly series and current closing weights; the response publishes their position/value coverage, so excluded illiquid or recent holdings are explicit. Contribution is to modelled volatility, not historical P&L. |
| `GET` | `/api/portfolio/decisions/?ownership_id=&on_date=` | What the system proposed, what was executed of it and the worst class drift then and now. Observed history, not a backtest. |
...
| `GET/POST/PATCH/DELETE` | `/api/portfolio/holdings/?position_id=` | Dated factsheet snapshots of a position's underlying holdings. Each row has name, optional ISIN/ticker, asset class, geography, sector and weight; a snapshot cannot exceed 100%, so partial factsheets remain honest. The latest snapshot at the requested date is used. |
| `GET` | `/api/portfolio/exposure/?ownership_id=&on_date=` | Aggregate exposure with coverage and source per dimension, class composition, concentration, approximate overlap by declared exposure and exact overlap where dated holdings share the same identifier (or, less reliably, the same normalized name). Holdings take precedence; manual declarations remain the fallback. With `ownership_id`, position and cash use the allocation perimeter and `classes.percent_basis=positions_and_cash`; without it, `positions_total` retains the whole-portfolio read. Class percentages include the unknown fraction as `unclassified`; geography/sector/vehicle keep their declared bases. |
...
| `POST` | `/api/portfolio/contribution/solve/` | Simulates the split of a contribution without saving anything. Optional `source_account_id` distinguishes external new money from cash already inside the scope. `quality` blocks incomplete valuation, FX, ownership, cash or class evidence and returns dated corrective actions; `funding` and `liquidity` declare available source, tactical reserve, minimum accumulation and operating remainder. `impact` publishes total/class amounts, weights and bands before/after; `review_token` binds those reviewed inputs. An external reserve requires a real linked cash destination. `unreachable` names the classes that have a target and no product to put the money in: their share ends up spread over the rest, and saying so is the difference between a decision and a surprise. `unmet_commitments` names what this contribution does not cover, with the yearly cost of breaking it — the figure that says whether it is worth contributing more this month. |
| `GET/POST` | `/api/portfolio/baskets/?ownership_id=&status=` | Contribution baskets. `POST` requires the `review_token` returned by the matching solve and rejects a changed amount, policy, date or evidence; nothing reaches accounting until confirmed. `GET` includes the reviewed impact and filters by scope and by status, because the screen asks what is still open in one scope, not for the whole history of proposals. |
...
| `GET` | `/api/portfolio/alerts/?on_date=` | Deterministic, user-scoped in-app attention queue. It groups typed actions for valuation/ownership/FX gaps, missing strategy or drift, concentration, exposure coverage and draft baskets; it never persists or sends notifications. |
...
| `GET` | `/api/portfolio/workspace/?container_id=&asset_class=&currency=&include_timeline=&include_position_details=&include_flows=` | Overview, performance, positions, timeline and quality for one period off a single context load. An inventory filter scopes **all** of them and the payload echoes the surviving position ids in `scope`; only the positions list stays whole, since the table filters client-side over the same set. `include_timeline=false` returns the same timeline envelope with an empty series and limits the context to the requested period; `include_position_details=false` returns the value/class rows needed by Resumen without per-position performance or attribution, declared by `position_details`; `include_flows=false` leaves the same return metrics but omits their technical flow-ledger rows. All options default to `true` for API compatibility. |
...
| `GET/PUT` | `/api/plan/asset-functions/` | Returns effective asset classification (inferred + override) and updates `PlanAssetFunction` overrides. Its `portfolio_valuation` coverage says whether linked investment Assets were valued from fresh/carrying Cartera positions or retained their Patrimonio fallback; values are replaced, never added. |
