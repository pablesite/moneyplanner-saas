...
function sumAnnualByType(entries, key, expected) {
    return entries.reduce(function (acc, entry) { var _a; return (entry[key] === expected ? acc + Number((_a = entry.amountAnnual) !== null && _a !== void 0 ? _a : 0) : acc); }, 0);
}
...
function computeAssetLiquidityMetrics(params) {
    var _a, _b;
    var summary = params.summary, activeAssets = params.activeAssets, assetsValue = params.assetsValue;
    var assetsByCategoryValues = Object.values((_a = summary === null || summary === void 0 ? void 0 : summary.assets_by_category) !== null && _a !== void 0 ? _a : {})
        .map(function (value) { return Math.max(0, toNumber(value)); })
    ...
    var topAssetShare = assetsValue > 0 ? ((_b = assetsByCategoryValues.sort(function (a, b) { return b - a; })[0]) !== null && _b !== void 0 ? _b : 0) / assetsValue : null;
    var diversificationIndex = (function () {
        if (assetsValue <= 0)
            return null;
        var shares = assetsByCategoryValues.map(function (value) { return value / assetsValue; });
        if (!shares.length)
            return null;
    ...
    return {
        ...
        immediateLiquidityShareWithinEmergency: emergencyLiquidAssetsValue > 0
...
function computeDebtMetrics(params) {
    var _a;
    var debtRows = params.debtRows, liabilitiesValue = params.liabilitiesValue, annualIncomeEntries = params.annualIncomeEntries;
    var topLiabilityShare = liabilitiesValue > 0
        ? ((_a = debtRows.map(function (row) { return row.amountBase; }).sort(function (a, b) { return b - a; })[0]) !== null && _a !== void 0 ? _a : 0) / liabilitiesValue
        : null;
    var taes = debtRows
    ...
    var weightedLiabilityTaePct = (function () {
        var rows = debtRows.filter(function (row) { return row.taePct != null; });
    ...
    var liabilitiesWithPaymentInputCount = debtRows.filter(function (row) { return row.hasMonthlyPaymentInput; }).length;
    var hasNoDebtPaymentInputs = liabilitiesWithAmountCount > 0 && liabilitiesWithPaymentInputCount === 0;
    var totalMonthlyDebtPayment = debtRows.reduce(function (acc, row) { return acc + row.monthlyPaymentAmount; }, 0);
    var recurrentAnnualIncome = sumAnnualByType(annualIncomeEntries, 'incomeType', 'recurrent');
    var monthlyIncome = Number.isFinite(recurrentAnnualIncome) && recurrentAnnualIncome > 0
        ? recurrentAnnualIncome / 12
        : null;
    var debtPaymentToIncome = liabilitiesValue <= 0
        ? 0
...
function computePhase3ExpenseCoverageMetrics(params) {
    var annualExpenseEntries = params.annualExpenseEntries, emergencyLiquidAssetsValue = params.emergencyLiquidAssetsValue;
    var structuralOperatingExpense = annualExpenseEntries.reduce(function (acc, entry) {
        var _a;
        if (expenseTimeProfile(entry) !== 'structural_recurrent')
            return acc;
        return expenseCashflowRole(entry) === 'operating' ? acc + Number((_a = entry.amountAnnual) !== null && _a !== void 0 ? _a : 0) : acc;
    }, 0);
    var temporaryCommitmentExpense = annualExpenseEntries.reduce(function (acc, entry) {
        var _a;
        if (expenseTimeProfile(entry) !== 'term_recurrent')
            return acc;
        return expenseCashflowRole(entry) === 'temporary_commitment'
            ? acc + Number((_a = entry.amountAnnual) !== null && _a !== void 0 ? _a : 0)
            : acc;
    }, 0);
    var structuralOperatingMonthlyExpense = structuralOperatingExpense > 0 ? structuralOperatingExpense / 12 : null;
    var currentCommittedMonthlyExpense = structuralOperatingExpense + temporaryCommitmentExpense > 0
        ? (structuralOperatingExpense + temporaryCommitmentExpense) / 12
        : null;
    return {
        emergencyCoverageMonthsBase: structuralOperatingMonthlyExpense != null && structuralOperatingMonthlyExpense > 0
            ? emergencyLiquidAssetsValue / structuralOperatingMonthlyExpense
            : null,
        emergencyCoverageMonthsCommitted: currentCommittedMonthlyExpense != null && currentCommittedMonthlyExpense > 0
            ? emergencyLiquidAssetsValue / currentCommittedMonthlyExpense
            : null,
    };
}
function computePhase2CashFlowAdjustedScore(input) {
    var incomeTimeProfile = function (entry) { var _a; return (_a = entry.timeProfile) !== null && _a !== void 0 ? _a : (entry.incomeType === 'one_off' ? 'one_off' : 'structural_recurrent'); };
    ...
    var structuralOperatingRatio = recurrentAnnualIncome > 0 ? structuralOperatingExpense / recurrentAnnualIncome : null;
    var committedLoadRatio = recurrentAnnualIncome > 0
        ? (structuralOperatingExpense + temporaryCommitmentExpense) / recurrentAnnualIncome
        : null;
    var temporaryCommitmentRatio = recurrentAnnualIncome > 0 ? temporaryCommitmentExpense / recurrentAnnualIncome : null;
    var oneOffIncome = input.annualIncomeEntries.reduce(function (acc, entry) { var _a; return incomeTimeProfile(entry) === 'one_off' ? acc + Number((_a = entry.amountAnnual) !== null && _a !== void 0 ? _a : 0) : acc; }, 0);
    var totalAnnualIncome = recurrentAnnualIncome + oneOffIncome;
    var totalAnnualExpense = input.annualExpenseEntries.reduce(function (acc, entry) { var _a; return acc + Number((_a = entry.amountAnnual) !== null && _a !== void 0 ? _a : 0); }, 0);
    var totalAnnualCashFlowRatio = recurrentAnnualIncome > 0
...
function computeGuidePhaseDiagnostics(input) {
    var _a;
    var summary = input.summary, assets = input.assets, liabilities = input.liabilities, annualIncomeEntries = input.annualIncomeEntries;
    var annualExpenseEntries = (_a = input.annualExpenseEntries) !== null && _a !== void 0 ? _a : [];
    var activeAssets = assets.filter(function (asset) { return asset.is_active; });
    var activeLiabilities = liabilities.filter(function (liability) { return liability.is_active; });
    var assetsValue = Math.max(0, toNumber(summary === null || summary === void 0 ? void 0 : summary.total_assets));
    var liabilitiesValue = Math.max(0, toNumber(summary === null || summary === void 0 ? void 0 : summary.total_liabilities));
    var unbackedDebtValue = Math.max(0, toNumber(summary === null || summary === void 0 ? void 0 : summary.liabilities_unbacked));
    var _b = computeAssetLiquidityMetrics({
        summary: summary,
        activeAssets: activeAssets,
        assetsValue: assetsValue,
    }), illiquidAssetsShare = _b.illiquidAssetsShare, emergencyLiquidAssetsValue = _b.emergencyLiquidAssetsValue, emergencyLiquidityToAssets = _b.emergencyLiquidityToAssets, immediateLiquidityShareWithinEmergency = _b.immediateLiquidityShareWithinEmergency, topAssetShare = _b.topAssetShare, diversificationIndex = _b.diversificationIndex;
    var unbackedDebtToAssets = assetsValue > 0 ? unbackedDebtValue / assetsValue : null;
    var unbackedDebtToLiabilities = liabilitiesValue > 0 ? unbackedDebtValue / liabilitiesValue : null;
    ...
    }), topLiabilityShare = _c.topLiabilityShare, maxLiabilityTaePct = _c.maxLiabilityTaePct, weightedLiabilityTaePct = _c.weightedLiabilityTaePct, hasNoDebtPaymentInputs = _c.hasNoDebtPaymentInputs, debtPaymentToIncome = _c.debtPaymentToIncome, highInterestDebtShare = _c.highInterestDebtShare;
    ...
    var _d = computePhase3ExpenseCoverageMetrics({
        annualExpenseEntries: annualExpenseEntries,
        emergencyLiquidAssetsValue: emergencyLiquidAssetsValue,
    }), emergencyCoverageMonthsBase = _d.emergencyCoverageMonthsBase, emergencyCoverageMonthsCommitted = _d.emergencyCoverageMonthsCommitted;
