...
function useAccountingPage() {
    ...
    var _a = (0, pinia_1.storeToRefs)(store), loading = _a.loading, accountCreationLoading = _a.accountCreationLoading, transactionCreationLoading = _a.transactionCreationLoading, importPreviewLoading = _a.importPreviewLoading, importCommitLoading = _a.importCommitLoading, error = _a.error;
    var _b = (0, pinia_1.storeToRefs)(store), accounts = _b.accounts, transactions = _b.transactions, monthlySummary = _b.monthlySummary, accountBalancesSummary = _b.accountBalancesSummary, moneyWizImportPreview = _b.moneyWizImportPreview, moneyWizImportCommitResult = _b.moneyWizImportCommitResult;
    ...
    var selectedYear = (0, vue_1.computed)({
        get: function () { return store.selectedYear; },
        set: function (value) {
            store.selectedYear = value;
        },
    });
    var selectedMonth = (0, vue_1.computed)({
        get: function () { return store.selectedMonth; },
        set: function (value) {
    ...
    var yearOptions = (0, vue_1.computed)(function () {
        ...
        return Array.from(values).sort(function (a, b) { return b - a; });
    });
    var accountMap = (0, vue_1.computed)(function () { return new Map(accounts.value.map(function (account) { return [account.id, account]; })); });
    var liquidityAccounts = (0, vue_1.computed)(function () {
        return accounts.value.filter(function (account) { return account.account_type === 'asset'; });
    });
    ...
    var availableManualAssetOptions = (0, vue_1.computed)(function () {
        return manualAssets.value.filter(function (asset) { return asset.is_active && asset.tracking_mode === 'manual'; });
    });
    var availableManualLiabilityOptions = (0, vue_1.computed)(function () {
        return manualLiabilities.value.filter(function (liability) { return liability.is_active && liability.tracking_mode === 'manual'; });
    });
    ...
    var assetNameById = (0, vue_1.computed)(function () { return new Map(manualAssets.value.map(function (asset) { var _a; return [asset.id, String((_a = asset.name) !== null && _a !== void 0 ? _a : '').trim()]; })); });
    var liabilityNameById = (0, vue_1.computed)(function () {
        return new Map(manualLiabilities.value.map(function (liability) {
            var _a;
            return [
                liability.id,
    ...
    var accountPositionMetaByAccountId = (0, vue_1.computed)(function () {
        ...
        accounts.value.forEach(function (account) {
            var _a, _b, _c, _d;
            if (account.asset_id != null) {
                var asset = manualAssets.value.find(function (row) { return row.id === account.asset_id; });
                if (asset) {
                    map.set(account.id, {
                        position_type: 'asset',
                        category: String((_a = asset.category) !== null && _a !== void 0 ? _a : '').trim() || 'other',
                        subcategory: String((_b = asset.subcategory) !== null && _b !== void 0 ? _b : '').trim() || 'other',
                    });
                    return;
                }
            }
            if (account.liability_id != null) {
                var liability = manualLiabilities.value.find(function (row) { return row.id === account.liability_id; });
                if (liability) {
                    map.set(account.id, {
                        position_type: 'liability',
                        category: String((_c = liability.category) !== null && _c !== void 0 ? _c : '').trim() || 'other',
                        subcategory: String((_d = liability.subcategory) !== null && _d !== void 0 ? _d : '').trim() || 'other',
                    });
                }
            }
        });
    ...
    var hasAvailableManualPositions = (0, vue_1.computed)(function () {
        return availableManualAssetOptions.value.length > 0 ||
    ...
    var incomeOptions = (0, vue_1.computed)(function () {
        return incomeStore.entries.value
            .filter(function (entry) { return entry.fiscalYear === selectedYear.value; })
    ...
    var transferCounterpartyOptions = (0, vue_1.computed)(function () {
        return liquidityAccounts.value.filter(function (account) { return account.id !== quickEntryForm.account_id; });
    });
    ...
    var liabilityCounterpartyOptions = (0, vue_1.computed)(function () {
        return accounts.value.filter(function (account) { return account.account_type === 'liability' && account.liability_id != null; });
    });
    var debtInterestOptions = (0, vue_1.computed)(function () {
        return accounts.value.filter(function (account) { return account.account_type === 'expense'; });
    });
    ...
    function debtPaymentBreakdownReady(amountValue) {
        ...
        if (quickEntryForm.liability_account_id == null)
            return false;
        if (principalValue <= 0 || interestValue < 0)
            return false;
        if (interestValue > 0 && quickEntryForm.interest_account_id == null)
            return false;
        if (interestValue > 0 && !hasQuickClassification())
            return false;
    ...
    var quickEntryReady = (0, vue_1.computed)(function () {
        if (!quickEntryForm.description.trim())
            return false;
        if (!quickEntryForm.booking_date || !quickEntryForm.value_date)
            return false;
        ...
        if ((quickEntryForm.movement_type === 'income' || quickEntryForm.movement_type === 'expense') &&
    ...
    var quickCategoryOptions = (0, vue_1.computed)(function () {
        if (quickEntryForm.movement_type === 'income')
            return data_input_1.incomeCategories;
        if (quickEntryForm.movement_type === 'expense')
    ...
    var quickSubcategoryOptions = (0, vue_1.computed)(function () {
        if (!quickEntryForm.category_key)
            return [];
        if (quickEntryForm.movement_type === 'income') {
            return data_input_1.incomeSubcategories.filter(function (row) { return row.category === quickEntryForm.category_key; });
        }
        if (quickEntryForm.movement_type === 'expense' ||
            (quickEntryForm.movement_type === 'debt_payment' &&
                toNumber(quickEntryForm.interest_amount) > 0)) {
            return data_input_1.expenseSubcategories.filter(function (row) { return row.category === quickEntryForm.category_key; });
        }
        return [];
    });
    var annualIncomeOptionsCompatible = (0, vue_1.computed)(function () {
        if (quickEntryForm.movement_type !== 'income')
            return [];
        if (!quickEntryForm.category_key || !quickEntryForm.subcategory_key)
            return [];
        return incomeOptions.value.filter(function (entry) {
            return entry.category === quickEntryForm.category_key &&
    ...
    var editAccountOptions = (0, vue_1.computed)(function () {
        return accounts.value
            .filter(function (account) { return account.account_type === 'asset' || account.account_type === 'liability'; })
            .sort(function (a, b) { return a.name.localeCompare(b.name, 'es'); });
    });
    ...
    var editKindNeedsCounterparty = (0, vue_1.computed)(function () { return isCounterpartyKind(editTransactionForm.kind); });
    ...
    var editCounterpartyOptions = (0, vue_1.computed)(function () {
        var baseOptions = editAccountOptions.value.filter(function (account) { return account.id !== editTransactionForm.account_id; });
        if (editTransactionForm.kind === 'investment_purchase') {
            return baseOptions.filter(function (account) { return account.account_type === 'asset' && account.asset_id != null; });
        }
        if (editTransactionForm.kind === 'debt_payment') {
            return baseOptions.filter(function (account) { return account.account_type === 'liability' && account.liability_id != null; });
        }
        return baseOptions;
    });
    var editCounterpartyMissingHint = (0, vue_1.computed)(function () {
        if (!editKindNeedsCounterparty.value)
            return '';
        if (editCounterpartyOptions.value.length > 0)
            return '';
        if (editTransactionForm.kind === 'investment_purchase') {
    ...
    var editSubcategoryOptions = (0, vue_1.computed)(function () {
        if (!editTransactionForm.category_key)
            return [];
        if (editTransactionForm.kind === 'income') {
            return data_input_1.incomeSubcategories.filter(function (row) { return row.category === editTransactionForm.category_key; });
        }
        if (editTransactionForm.kind === 'expense') {
            return data_input_1.expenseSubcategories.filter(function (row) { return row.category === editTransactionForm.category_key; });
        }
        return [];
    });
    var editEntryReady = (0, vue_1.computed)(function () {
        if (!editTransactionForm.description.trim())
            return false;
        if (!editTransactionForm.booking_date || !editTransactionForm.value_date)
            return false;
        if (editTransactionForm.account_id == null)
            return false;
    ...
    (0, vue_1.watch)(function () { return transactionForm.entries.map(function (entry) { return entry.account_id; }); }, function (accountIds) {
    ...
    (0, vue_1.watch)(function () { return quickEntryForm.movement_type; }, function (movementType) {
        ...
        var remembered = movementType === 'income' || movementType === 'expense' || movementType === 'debt_payment'
    ...
    (0, vue_1.watch)(function () { return quickEntryForm.category_key; }, function () {
        if (quickEntryForm.subcategory_key &&
            !quickSubcategoryOptions.value.some(function (row) { return row.value === quickEntryForm.subcategory_key; })) {
            quickEntryForm.subcategory_key = '';
            quickEntryForm.annual_income_entry_id = null;
            quickEntryForm.annual_expense_entry_id = null;
        }
    });
    (0, vue_1.watch)(function () { return quickEntryForm.subcategory_key; }, function (value) {
        if (!value)
            return;
        if (quickEntryForm.movement_type === 'income') {
    ...
    (0, vue_1.watch)(function () { return editTransactionForm.kind; }, function (kind) {
        ...
        if (isCounterpartyKind(kind)) {
            if (editTransactionForm.counterparty_account_id == null &&
    ...
    (0, vue_1.watch)(function () { return editTransactionForm.account_id; }, function (accountId) {
        var _a, _b;
        if (accountId == null) {
        ...
        if (!isCounterpartyKind(editTransactionForm.kind))
            return;
        if (editTransactionForm.counterparty_account_id === accountId) {
            editTransactionForm.counterparty_account_id = (_b = (_a = editCounterpartyOptions.value[0]) === null || _a === void 0 ? void 0 : _a.id) !== null && _b !== void 0 ? _b : null;
        }
    });
    (0, vue_1.watch)(function () { return editTransactionForm.category_key; }, function () {
        if (editTransactionForm.subcategory_key &&
            !editSubcategoryOptions.value.some(function (row) { return row.value === editTransactionForm.subcategory_key; })) {
            editTransactionForm.subcategory_key = '';
        }
    });
    (0, vue_1.watch)(function () { return activationForm.position_type; }, function () {
        activationForm.position_id = null;
    });
    var debitTotal = (0, vue_1.computed)(function () {
        return transactionForm.entries
            .filter(function (entry) { return entry.side === 'debit'; })
            .reduce(function (sum, entry) { return sum + toNumber(entry.amount); }, 0);
    });
    var creditTotal = (0, vue_1.computed)(function () {
        return transactionForm.entries
            .filter(function (entry) { return entry.side === 'credit'; })
            .reduce(function (sum, entry) { return sum + toNumber(entry.amount); }, 0);
    });
    var transactionBalanced = (0, vue_1.computed)(function () {
        return transactionForm.entries.length >= 2 &&
            debitTotal.value > 0 &&
            debitTotal.value === creditTotal.value;
    });
    var summaryRows = (0, vue_1.computed)(function () {
        var _a, _b;
        return ((_b = (_a = monthlySummary.value) === null || _a === void 0 ? void 0 : _a.months) !== null && _b !== void 0 ? _b : []).map(function (row) { return (__assign(__assign({}, row), { incomeValue: toNumber(row.income_total), expenseValue: toNumber(row.expense_total), uncategorizedValue: toNumber(row.uncategorized_total) })); });
    });
    var liquidityBalanceRows = (0, vue_1.computed)(function () {
        var _a, _b;
        return ((_b = (_a = accountBalancesSummary.value) === null || _a === void 0 ? void 0 : _a.accounts) !== null && _b !== void 0 ? _b : []).map(function (row) { return (__assign(__assign({}, row), { currentBalanceValue: toNumber(row.current_balance), periodDebitValue: toNumber(row.period_debit_total), periodCreditValue: toNumber(row.period_credit_total), periodNetChangeValue: toNumber(row.period_net_change) })); });
    });
    var liquidityBalanceTotal = (0, vue_1.computed)(function () { var _a, _b; return toNumber((_b = (_a = accountBalancesSummary.value) === null || _a === void 0 ? void 0 : _a.totals_by_account_type.asset) !== null && _b !== void 0 ? _b : '0'); });
        ...
        return transactions.value.filter(function (transaction) {
            ...
            if (normalizedQuery) {
                var haystack = __spreadArray([
                    ...
                    transaction.notes
                ], transaction.entries.map(function (entry) { return "".concat(entry.account_name, " ").concat(entry.notes); }), true).join(' ')
            ...
            if (activityFilters.accountId !== 'all') {
                ...
                if (!transaction.entries.some(function (entry) { return entry.account_id === expectedAccountId_1; }))
                    return false;
            }
            if (activityFilters.kind !== 'all') {
                if (getTransactionActivityKind(transaction) !== activityFilters.kind)
                    return false;
            }
            return true;
        });
    ...
    var cuentasSelectedAccount = (0, vue_1.computed)(function () {
        ...
        return cuentasSelectedAccountId.value != null
            ? (_a = accounts.value.find(function (a) { return a.id === cuentasSelectedAccountId.value; })) !== null && _a !== void 0 ? _a : null
            : null;
    });
    var cuentasRawTransactions = (0, vue_1.computed)(function () {
        ...
        return transactions.value
            .filter(function (t) {
            return t.entries.some(function (e) { return e.account_id === accountId; }) &&
            ...
            var impactValue = t.entries
                .filter(function (e) { return e.account_id === accountId; })
                .reduce(function (sum, e) { return sum + signedImpact(account.account_type, e.side, e.amount); }, 0);
            return __assign(__assign({}, t), { impactValue: impactValue, tone: impactTone(impactValue) });
        });
    });
    ...
    var cuentasHasMore = (0, vue_1.computed)(function () { return cuentasVisibleCount.value < cuentasRawTransactions.value.length; });
    ...
    var todosRawTransactions = (0, vue_1.computed)(function () {
        return filteredTransactions.value.filter(function (t) {
            return (!todosDateFrom.value || t.booking_date >= todosDateFrom.value) &&
    ...
    var todosHasMore = (0, vue_1.computed)(function () { return todosVisibleCount.value < todosRawTransactions.value.length; });
    ...
    function transactionMainAmount(t) {
        var debitTotal = t.entries
            .filter(function (e) { return e.side === 'debit'; })
            .reduce(function (sum, e) { return sum + toNumber(e.amount); }, 0);
        return debitTotal;
    }
    ...
    function toEditableKind(transaction) {
        ...
        if (detected === 'expense')
            return 'expense';
        if (detected === 'investment_purchase')
            return 'investment_purchase';
        if (detected === 'debt_payment')
            return 'debt_payment';
        return 'transfer';
    }
    ...
    function resolveEditAccountsForKind(transaction, kind, debitEntry, creditEntry) {
        ...
        if (kind === 'income') {
            return {
                accountId: (_a = debitEntry === null || debitEntry === void 0 ? void 0 : debitEntry.account_id) !== null && _a !== void 0 ? _a : null,
                counterpartyAccountId: null,
            };
        }
        if (kind === 'expense') {
            return {
                accountId: (_b = creditEntry === null || creditEntry === void 0 ? void 0 : creditEntry.account_id) !== null && _b !== void 0 ? _b : null,
                counterpartyAccountId: null,
            };
        }
        if (kind === 'debt_payment') {
            var liabilityEntry = (_d = (_c = transaction.entries.find(function (entry) { var _a; return ((_a = accountMap.value.get(entry.account_id)) === null || _a === void 0 ? void 0 : _a.account_type) === 'liability'; })) !== null && _c !== void 0 ? _c : transaction.entries.find(function (entry) { return entry.side === 'debit'; })) !== null && _d !== void 0 ? _d : debitEntry;
            return {
                accountId: (_e = creditEntry === null || creditEntry === void 0 ? void 0 : creditEntry.account_id) !== null && _e !== void 0 ? _e : null,
                counterpartyAccountId: (_f = liabilityEntry === null || liabilityEntry === void 0 ? void 0 : liabilityEntry.account_id) !== null && _f !== void 0 ? _f : null,
            };
        }
        return {
            accountId: (_g = creditEntry === null || creditEntry === void 0 ? void 0 : creditEntry.account_id) !== null && _g !== void 0 ? _g : null,
            counterpartyAccountId: (_h = debitEntry === null || debitEntry === void 0 ? void 0 : debitEntry.account_id) !== null && _h !== void 0 ? _h : null,
        };
    }
    function fillEditTransactionForm(transaction) {
        var _a, _b, _c, _d, _e, _f, _g, _h, _j, _k;
        var primaryClassifiedEntry = (_a = transaction.entries.find(function (entry) {
            return Boolean(entry.flow_family) &&
        ...
        editTransactionPersistedEntries.value = transaction.entries.map(function (entry) {
            ...
            return ({
                ...
                annual_income_entry_id: (_d = entry.annual_income_entry_id) !== null && _d !== void 0 ? _d : null,
                annual_expense_entry_id: (_e = entry.annual_expense_entry_id) !== null && _e !== void 0 ? _e : null,
        ...
        editTransactionForm.currency = (_d = (_c = transaction.entries[0]) === null || _c === void 0 ? void 0 : _c.currency) !== null && _d !== void 0 ? _d : 'EUR';
        ...
        var debitEntry = (_f = (_e = transaction.entries.find(function (entry) { return entry.side === 'debit'; })) !== null && _e !== void 0 ? _e : transaction.entries[0]) !== null && _f !== void 0 ? _f : null;
        var creditEntry = (_h = (_g = transaction.entries.find(function (entry) { return entry.side === 'credit'; })) !== null && _g !== void 0 ? _g : transaction.entries[1]) !== null && _h !== void 0 ? _h : null;
        var _l = resolveEditAccountsForKind(transaction, kind, debitEntry, creditEntry), accountId = _l.accountId, counterpartyAccountId = _l.counterpartyAccountId;
        editTransactionForm.account_id = accountId;
        editTransactionForm.counterparty_account_id = counterpartyAccountId;
        editTransactionForm.category_key = (_j = primaryClassifiedEntry === null || primaryClassifiedEntry === void 0 ? void 0 : primaryClassifiedEntry.category_key) !== null && _j !== void 0 ? _j : '';
        editTransactionForm.subcategory_key = (_k = primaryClassifiedEntry === null || primaryClassifiedEntry === void 0 ? void 0 : primaryClassifiedEntry.subcategory_key) !== null && _k !== void 0 ? _k : '';
        editTransactionForm.kind_label = activityKindDisplay(kind);
    }
    function scaleEntriesToAmount(entries, targetAmount) {
        var scaled = entries.map(function (entry) { return (__assign({}, entry)); });
        var debitIndexes = scaled
            .map(function (entry, index) { return (entry.side === 'debit' ? index : -1); })
            .filter(function (index) { return index >= 0; });
        var creditIndexes = scaled
            .map(function (entry, index) { return (entry.side === 'credit' ? index : -1); })
            .filter(function (index) { return index >= 0; });
        var currentDebitTotal = debitIndexes.reduce(function (sum, index) { return sum + toNumber(scaled[index].amount); }, 0);
    ...
    function setEditedKindOnEntries(entries, kind, categoryKey, subcategoryKey) {
        var _a, _b;
        var nextEntries = entries.map(function (entry) { return (__assign(__assign({}, entry), { flow_family: '', category_key: '', subcategory_key: '', annual_income_entry_id: null, annual_expense_entry_id: null, asset_id: null, liability_id: null })); });
        if (!isClassificationKind(kind))
            return nextEntries;
        var preferredSide = kind === 'income' ? 'credit' : 'debit';
        var preferredEntry = (_b = (_a = nextEntries.find(function (entry) { return entry.side === preferredSide; })) !== null && _a !== void 0 ? _a : nextEntries[0]) !== null && _b !== void 0 ? _b : null;
        if (!preferredEntry)
            return nextEntries;
    ...
    function setEditedAccountsOnEntries(entries, kind, accountId, counterpartyAccountId) {
        var _a, _b, _c, _d;
        var nextEntries = entries.map(function (entry) { return (__assign({}, entry)); });
        var debitEntry = (_b = (_a = nextEntries.find(function (entry) { return entry.side === 'debit'; })) !== null && _a !== void 0 ? _a : nextEntries[0]) !== null && _b !== void 0 ? _b : null;
        var creditEntry = (_d = (_c = nextEntries.find(function (entry) { return entry.side === 'credit'; })) !== null && _c !== void 0 ? _c : nextEntries[1]) !== null && _d !== void 0 ? _d : null;
    ...
    function resolveClassificationCounterpartyAccountId(kind, currency) {
        ...
        var candidates = accounts.value.filter(function (account) { return account.account_type === expectedType; });
        if (!candidates.length)
            return null;
        return ((_h = (_f = (_d = (_b = (_a = candidates.find(function (account) { return account.currency === currency && account.origin === 'system'; })) === null || _a === void 0 ? void 0 : _a.id) !== null && _b !== void 0 ? _b : (_c = candidates.find(function (account) { return account.currency === currency; })) === null || _c === void 0 ? void 0 : _c.id) !== null && _d !== void 0 ? _d : (_e = candidates.find(function (account) { return account.origin === 'system'; })) === null || _e === void 0 ? void 0 : _e.id) !== null && _f !== void 0 ? _f : (_g = candidates[0]) === null || _g === void 0 ? void 0 : _g.id) !== null && _h !== void 0 ? _h : null);
    }
        ...
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 1:
                        _a.trys.push([1, 4, , 5]);
                        return [4 /*yield*/, api_1.coreAccountingApi.createAccount({
                                name: defaultName,
    ...
    function resolveAdjustmentCounterpartyAccountId(currency) {
        var _a, _b, _c, _d, _e, _f, _g, _h;
        var candidates = accounts.value.filter(function (account) { return account.account_type === 'equity'; });
        if (!candidates.length)
            return null;
        return ((_h = (_f = (_d = (_b = (_a = candidates.find(function (account) { return account.currency === currency && account.origin === 'system'; })) === null || _a === void 0 ? void 0 : _a.id) !== null && _b !== void 0 ? _b : (_c = candidates.find(function (account) { return account.currency === currency; })) === null || _c === void 0 ? void 0 : _c.id) !== null && _d !== void 0 ? _d : (_e = candidates.find(function (account) { return account.origin === 'system'; })) === null || _e === void 0 ? void 0 : _e.id) !== null && _f !== void 0 ? _f : (_g = candidates[0]) === null || _g === void 0 ? void 0 : _g.id) !== null && _h !== void 0 ? _h : null);
    }
    ...
    function accountDeltaSide(accountType, delta) {
        ...
        if (delta >= 0)
            return debitIncreases ? 'debit' : 'credit';
    ...
    function buildBalanceAdjustmentEntries(amount, targetAccount, counterpartyAccount) {
        ...
        var makeEntry = function (account, side) { return ({
    ...
    function resolveEditedTransactionEntries(parsedAmount, selectedAccount) {
        return __awaiter(this, void 0, void 0, function () {
            var targetBalance, currentBalance, delta, counterpartyId, counterpartyAccount, editedAmount, classificationCounterpartyAccountId, _a, scaledEntries, kindAdjustedEntries;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    ...
                    case 1:
                        ...
                        return [2 /*return*/, buildBalanceAdjustmentEntries(delta, selectedAccount, counterpartyAccount)];
                    case 2:
                        ...
                        return [3 /*break*/, 5];
                    case 3: return [4 /*yield*/, ensureClassificationCounterpartyAccountId(editTransactionForm.kind, selectedAccount.currency)];
                    case 4:
                        _a = _b.sent();
                        _b.label = 5;
                    case 5:
                        ...
                        scaledEntries = scaleEntriesToAmount(editTransactionPersistedEntries.value, editedAmount);
                        kindAdjustedEntries = editTransactionForm.kind === editTransactionForm.initial_kind
                            ? scaledEntries
                            : setEditedKindOnEntries(scaledEntries, editTransactionForm.kind, editTransactionForm.category_key, editTransactionForm.subcategory_key);
                        return [2 /*return*/, setEditedAccountsOnEntries(kindAdjustedEntries, editTransactionForm.kind, editTransactionForm.account_id, classificationCounterpartyAccountId)];
                }
            });
        });
    }
    function getTransactionActivityKind(transaction) {
        var hasIncomeClassified = transaction.entries.some(function (entry) { return entry.flow_family === 'income'; });
        if (hasIncomeClassified)
            return 'income';
        var hasExpenseClassified = transaction.entries.some(function (entry) { return entry.flow_family === 'expense'; });
        var hasIncomeLink = transaction.entries.some(function (entry) { return entry.annual_income_entry_id != null; });
        if (hasIncomeLink)
            return 'income';
        var hasExpenseLink = transaction.entries.some(function (entry) { return entry.annual_expense_entry_id != null; });
        var hasLiabilityLink = transaction.entries.some(function (entry) { return entry.liability_id != null; });
        if (hasLiabilityLink)
            return 'debt_payment';
        if (hasExpenseClassified)
            return 'expense';
        if (hasExpenseLink)
            return 'expense';
        var assetEntries = transaction.entries.filter(function (entry) { var _a; return ((_a = accountMap.value.get(entry.account_id)) === null || _a === void 0 ? void 0 : _a.account_type) === 'asset'; });
        var hasAssetPositionLink = transaction.entries.some(function (entry) { return entry.asset_id != null; });
        if (hasAssetPositionLink)
            return 'investment_purchase';
    ...
    function removeEntry(key) {
        if (transactionForm.entries.length <= 2)
            return;
        transactionForm.entries = transactionForm.entries.filter(function (entry) { return entry.key !== key; });
    }
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    ...
                    case 1:
                        ...
                        if (activationForm.position_id != null &&
                            !availableManualPositionOptions.value.some(function (row) { return row.id === activationForm.position_id; })) {
                            activationForm.position_id = null;
                        }
                        return [3 /*break*/, 3];
                    case 2:
        ...
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        successMessage.value = null;
                        return [4 /*yield*/, store.createAccount({
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        if (activationForm.position_id == null)
                            return [2 /*return*/];
                        return [4 /*yield*/, activateNetWorthPositions(activationForm.position_type, [activationForm.position_id])];
                    case 1:
                        _a.sent();
                        return [2 /*return*/];
                }
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 1:
                        ...
                        return [4 /*yield*/, Promise.all(positionIds.map(function (positionId) {
                    ...
                    case 2:
                        _a.sent();
                        return [3 /*break*/, 5];
                    case 3: return [4 /*yield*/, Promise.all(positionIds.map(function (positionId) {
                    ...
                    case 6:
                        ...
                        successMessage.value =
                            positionIds.length === 1
                                ...
                                : "Tracking contable activado para ".concat(positionIds.length, " posiciones seleccionadas.");
                        return [3 /*break*/, 9];
                    case 7:
                        error_4 = _a.sent();
    ...
    function removeNetWorthTracking(account) {
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        targetType = account.asset_id != null ? 'asset' : account.liability_id != null ? 'liability' : null;
                        targetId = (_a = account.asset_id) !== null && _a !== void 0 ? _a : account.liability_id;
                        if (!targetType || targetId == null)
                            return [2 /*return*/];
                        successMessage.value = null;
                        if (!confirm("Quitar tracking contable de \"".concat(account.name, "\"?\n\n") +
                    ...
                    case 1:
                        ...
                        return [4 /*yield*/, api_2.coreNetWorthApi.updateAsset(targetId, { tracking_mode: 'manual' })];
                    case 2:
                        _b.sent();
                        return [3 /*break*/, 5];
                    case 3: return [4 /*yield*/, api_2.coreNetWorthApi.updateLiability(targetId, { tracking_mode: 'manual' })];
                    case 4:
                        _b.sent();
                        _b.label = 5;
                    case 5: return [4 /*yield*/, api_1.coreAccountingApi.updateAccount(account.id, {
                            is_active: false,
                            asset_id: null,
                            liability_id: null,
                        })];
        ...
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        successMessage.value = null;
                        if (!confirm("Eliminar cuenta \"".concat(accountName, "\"?\n\n") +
    ...
    function openTransactionForEditing(transactionId) {
        var transaction = transactions.value.find(function (row) { return row.id === transactionId; });
        if (!transaction)
            return false;
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        ...
                        if (!editTransactionPersistedEntries.value.length)
                            return [2 /*return*/, false];
                        validated = validateEditedTransactionInput();
                        if (!validated)
                            return [2 /*return*/, false];
                        return [4 /*yield*/, resolveEditedTransactionEntries(validated.parsedAmount, validated.selectedAccount)];
                    case 1:
                        payloadEntries = _b.sent();
                        if (!(payloadEntries === null || payloadEntries === void 0 ? void 0 : payloadEntries.length)) {
                        ...
                        payload = {
                            ...
                            entries: payloadEntries.map(function (entry) { return ({
                                account_id: entry.account_id,
                                side: entry.side,
                                amount: formatDecimalInput(entry.amount),
                                currency: entry.currency.trim().toUpperCase(),
    ...
    function deleteTransaction(transactionId, transactionDescription) {
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        transaction = transactions.value.find(function (row) { return row.id === transactionId; });
                        if ((transaction === null || transaction === void 0 ? void 0 : transaction.origin) === 'system') {
                            store.error = 'Los asientos de origen system no se pueden eliminar desde esta vista.';
                            return [2 /*return*/];
                        }
                        successMessage.value = null;
                        if (!confirm("Eliminar movimiento \"".concat(transactionDescription, "\"?\n\n") +
                            'La accion es irreversible y puede afectar saldos e historico.')) {
                            return [2 /*return*/];
                        }
        ...
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        successMessage.value = null;
                        payload = __assign(__assign(__assign(__assign(__assign(__assign({ movement_type: quickEntryForm.movement_type, booking_date: quickEntryForm.booking_date, value_date: quickEntryForm.value_date, description: quickEntryForm.description.trim(), amount: formatDecimalInput(quickEntryForm.amount), account_id: (_a = quickEntryForm.account_id) !== null && _a !== void 0 ? _a : 0, notes: quickEntryForm.notes.trim(), status: 'posted', origin: 'manual' }, (quickEntryNeedsClassification.value
                            ? {
                                flow_family: quickEntryForm.movement_type === 'income' ? 'income' : 'expense',
                                category_key: quickEntryForm.category_key,
                            ...
                            ? { counterparty_account_id: quickEntryForm.counterparty_account_id }
                            : {})), (quickEntryForm.movement_type === 'debt_payment'
                            ? __assign(__assign({ liability_account_id: quickEntryForm.liability_account_id, principal_amount: formatDecimalInput(quickEntryForm.principal_amount), interest_amount: formatDecimalInput(quickEntryForm.interest_amount || '0') }, (quickEntryForm.annual_expense_entry_id != null
                                ? { annual_expense_entry_id: quickEntryForm.annual_expense_entry_id }
                                : {})), (toNumber(quickEntryForm.interest_amount) > 0
                    ...
                    case 1:
                        ...
                        successMessage.value =
                            result.created_count > 0
                                ? "Importacion MoneyWiz completada: ".concat(result.created_count, " movimientos nuevos.")
