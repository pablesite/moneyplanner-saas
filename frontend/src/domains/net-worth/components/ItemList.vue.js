...
var isLiabilitiesList = (0, vue_1.computed)(function () { return props.title === 'Pasivos'; });
var ownershipLabel = function (o) {
    ...
    var parts = splits.map(function (s) {
        ...
        return "".concat(name, " ").concat((_a = s.percent) !== null && _a !== void 0 ? _a : '', "%").trim();
    });
...
var memberFilterOptions = (0, vue_1.computed)(function () {
    var list = Array.isArray(props.ownerships) ? props.ownerships : [];
    var members = list
        .filter(function (o) { return o.kind === 'individual' && o.member; })
        .map(function (o) { return o.member; })
        .filter(function (m) { return m != null; });
    var unique = new Map();
    for (var _i = 0, members_1 = members; _i < members_1.length; _i++) {
        var m = members_1[_i];
        if (!unique.has(m.id))
            unique.set(m.id, { id: m.id, name: m.name });
    }
    return Array.from(unique.values()).sort(function (a, b) { return a.name.localeCompare(b.name); });
});
var ownershipById = (0, vue_1.computed)(function () {
    var _a;
    var m = new Map();
    ((_a = props.ownerships) !== null && _a !== void 0 ? _a : []).forEach(function (o) { return m.set(o.id, o); });
    return m;
});
function ownershipShortLabel(ownershipRef) {
    ...
    if (!o)
        return "own #".concat(ownershipRef);
    if (o.kind === 'individual') {
        var name_1 = (_b = (_a = o.member) === null || _a === void 0 ? void 0 : _a.name) !== null && _b !== void 0 ? _b : "#".concat(ownershipRef);
        return name_1;
    }
    var splits = Array.isArray(o.splits) ? o.splits : [];
    if (!splits.length)
    ...
    var first = splits[0];
    var second = splits[1];
    var isAllHalf = splits.length === 2 &&
        first != null &&
        second != null &&
        String(first.percent) === '50.00' &&
        String(second.percent) === '50.00';
    var names = splits.map(function (s) { var _a, _b; return (_b = (_a = s.member) === null || _a === void 0 ? void 0 : _a.name) !== null && _b !== void 0 ? _b : '?'; });
    if (isAllHalf)
        return names.join('/');
    var parts = splits.map(function (s) { var _a, _b; return "".concat((_b = (_a = s.member) === null || _a === void 0 ? void 0 : _a.name) !== null && _b !== void 0 ? _b : '?', " ").concat(s.percent, "%"); });
    return parts.join(' | ');
}
...
var assetsById = (0, vue_1.computed)(function () {
    var _a;
    var m = new Map();
    ((_a = props.assets) !== null && _a !== void 0 ? _a : []).forEach(function (a) { return m.set(a.id, a); });
    return m;
});
...
function getMaxDecimals(currency) {
    var _a;
    return (_a = decimalsByCurrency[String(currency !== null && currency !== void 0 ? currency : '')]) !== null && _a !== void 0 ? _a : 2;
}
...
function clampDecimals(amount, maxDecimals) {
    ...
    if ((s.match(/\./g) || []).length > 1)
        return s;
    var _a = s.split('.'), _b = _a[0], i = _b === void 0 ? '' : _b, _c = _a[1], d = _c === void 0 ? '' : _c;
...
var amountError = (0, vue_1.computed)(function () {
    ...
    if (Number.isNaN(Number(normalizeAmountInput(clamped))))
        return 'Importe invalido';
    return '';
});
var categoryLabel = function (key) {
    var _a;
    var found = props.categories.find(function (c) { return c.value === key; });
    return (_a = found === null || found === void 0 ? void 0 : found.label) !== null && _a !== void 0 ? _a : key;
};
var subcategoryLabel = function (key) {
    var _a, _b;
    var list = (_a = props.subcategories) !== null && _a !== void 0 ? _a : [];
    var found = list.find(function (c) { return c.value === key; });
    return (_b = found === null || found === void 0 ? void 0 : found.label) !== null && _b !== void 0 ? _b : key;
};
...
var filteredItems = (0, vue_1.computed)(function () {
    ...
    var list = props.showArchived ? rawList : rawList.filter(function (it) { return it.is_active !== false; });
    if (ownershipFilter.value === 'all')
        return list;
    if (ownershipFilter.value === 'unassigned') {
        return list.filter(function (it) { return it.ownership_ref == null; });
    }
    ...
    for (var _i = 0, list_1 = list; _i < list_1.length; _i++) {
        var it = list_1[_i];
        if (it.ownership_ref == null)
            continue;
        var o = ownershipById.value.get(it.ownership_ref);
        if (!o)
            continue;
        var pct = 0;
        if (o.kind === 'individual' && ((_a = o.member) === null || _a === void 0 ? void 0 : _a.id) === memberId) {
            pct = 100;
        }
        else if (o.kind === 'shared') {
            var split = ((_b = o.splits) !== null && _b !== void 0 ? _b : []).find(function (s) { var _a; return ((_a = s.member) === null || _a === void 0 ? void 0 : _a.id) === memberId; });
            if (split)
                pct = Number(normalizeAmountInput(split.percent));
        }
        if (!pct || pct <= 0)
            continue;
        var sourceAmount = it.effective_amount != null && String(it.effective_amount).trim() !== ''
            ? toNumberAmount(String(it.effective_amount))
            : toNumberAmount(String(it.amount));
        var displayAmount_1 = sourceAmount * (pct / 100);
        out.push(__assign(__assign({}, it), { _displayAmount: displayAmount_1, _sharePercent: pct, _source: it }));
    }
    return out;
});
var grouped = (0, vue_1.computed)(function () {
    ...
    var assetOrder = new Map(orderedAssetCats.map(function (k, i) { return [k, i]; }));
    var liabOrder = new Map(orderedLiabCats.map(function (k, i) { return [k, i]; }));
    return Array.from(map.entries())
        ...
        var base = {
            ...
            items: items.sort(function (x, y) { return x.name.localeCompare(y.name); }),
            subgroups: [],
            hasSubgroups: false,
        };
        ...
        base.subgroups = Array.from(subMap.entries())
            ...
            .map(function (_a) {
            var subcategory = _a[0], subitems = _a[1];
            return ({
                subcategory: subcategory,
                label: subcategoryLabel(subcategory),
                items: subitems.sort(function (x, y) { return x.name.localeCompare(y.name); }),
            });
        });
...
function shouldShowGroupDetails(group) {
    if (group.hasSubgroups)
        return expandedCats.value.has(group.category);
...
var headerTotals = (0, vue_1.computed)(function () { return categoryTotals(filteredItems.value); });
var headerBaseValue = (0, vue_1.computed)(function () { return totalBaseAll(); });
...
function categoryTotals(items) {
    ...
    for (var _i = 0, items_2 = items; _i < items_2.length; _i++) {
        var it = items_2[_i];
        var cur = it.currency || '???';
        acc[cur] = ((_a = acc[cur]) !== null && _a !== void 0 ? _a : 0) + toNumberAmount(String(displayAmount(it)));
    }
    return acc;
}
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    if (!confirm("Eliminar este ".concat(label, " de forma permanente? Esta accion no se puede deshacer."))) {
                        return [2 /*return*/];
                    }
...
(0, vue_1.watch)(function () { return [draft.value.currency, draft.value.category]; }, function () {
    var _a;
    if (!((_a = draft.value) === null || _a === void 0 ? void 0 : _a.amount))
        return;
    var max = getMaxDecimals(draft.value.currency);
    draft.value.amount = trimTrailingZeros(clampDecimals(draft.value.amount, max));
});
(0, vue_1.watch)(function () { return [ownershipFilter.value, filteredItems.value.length]; }, function () {
    expandedCats.value = new Set();
});
(0, vue_1.watch)(function () { return props.ownershipFilterValue; }, function (next) {
    var normalized = normalizeOwnershipFilterValue(next);
    if (ownershipFilter.value !== normalized) {
        ownershipFilter.value = normalized;
    }
});
(0, vue_1.watch)(ownershipFilter, function (next) {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    if (!isValidAmountString(clamped))
                        return [2 /*return*/];
                    payload = __assign(__assign({}, draft.value), { amount: clamped, subcategory: draft.value.subcategory || undefined });
...
__VLS_asFunctionalElement1(__VLS_intrinsics.div, __VLS_intrinsics.div)(__assign({ class: "card ui-pro-panel ui-nw-list-panel" }));
/** @type {__VLS_StyleScopedClasses['card']} */ ;
/** @type {__VLS_StyleScopedClasses['ui-pro-panel']} */ ;
/** @type {__VLS_StyleScopedClasses['ui-nw-list-panel']} */ ;
...
if (__VLS_ctx.showOwnershipFilter !== false) {
    __VLS_asFunctionalElement1(__VLS_intrinsics.select, __VLS_intrinsics.select)(__assign({ value: (__VLS_ctx.ownershipFilter) }, { class: "select nw-select-sm" }));
...
if (__VLS_ctx.onAdd) {
    __VLS_asFunctionalElement1(__VLS_intrinsics.button, __VLS_intrinsics.button)(__assign(__assign({ onClick: (__VLS_ctx.onAdd) }, { class: "btn btn-primary btn-sm nw-list-add-icon-only" }), { type: "button", 'aria-label': "Anadir" }));
...
if (!__VLS_ctx.items.length) {
    ...
    __VLS_asFunctionalElement1(__VLS_intrinsics.div, __VLS_intrinsics.div)(__assign({ class: "grid gap-4" }));
    /** @type {__VLS_StyleScopedClasses['grid']} */ ;
    /** @type {__VLS_StyleScopedClasses['gap-4']} */ ;
    var _loop_1 = function (g) {
        __VLS_asFunctionalElement1(__VLS_intrinsics.section, __VLS_intrinsics.section)(__assign(__assign({ key: (g.category) }, { class: "nw-cat-block" }), { class: (__VLS_ctx.categoryClass(g.category)) }));
        /** @type {__VLS_StyleScopedClasses['nw-cat-block']} */ ;
        var __VLS_0 = ItemCategoryHeader_vue_1.default;
        // @ts-ignore
        var __VLS_1 = __VLS_asFunctionalComponent1(__VLS_0, new __VLS_0(__assign({ 'onToggle': {} }, { label: (g.label), count: (g.items.length), totalsLine: (__VLS_ctx.formatTotalsLine(__VLS_ctx.categoryTotals(g.items))), baseLabel: (__VLS_ctx.baseTotalLabel(g.category, g.items)), percent: (__VLS_ctx.categoryPercent(g.category, g.items)), showBaseTotal: (__VLS_ctx.shouldShowBaseTotal(__VLS_ctx.categoryTotals(g.items), __VLS_ctx.categoryBaseValue(g.category, g.items))), showToggle: (g.hasSubgroups || __VLS_ctx.isLiabilitiesList), expanded: (__VLS_ctx.expandedCats.has(g.category)) })));
        var __VLS_2 = __VLS_1.apply(void 0, __spreadArray([__assign({ 'onToggle': {} }, { label: (g.label), count: (g.items.length), totalsLine: (__VLS_ctx.formatTotalsLine(__VLS_ctx.categoryTotals(g.items))), baseLabel: (__VLS_ctx.baseTotalLabel(g.category, g.items)), percent: (__VLS_ctx.categoryPercent(g.category, g.items)), showBaseTotal: (__VLS_ctx.shouldShowBaseTotal(__VLS_ctx.categoryTotals(g.items), __VLS_ctx.categoryBaseValue(g.category, g.items))), showToggle: (g.hasSubgroups || __VLS_ctx.isLiabilitiesList), expanded: (__VLS_ctx.expandedCats.has(g.category)) })], __VLS_functionalComponentArgsRest(__VLS_1), false));
        var __VLS_5 = void 0;
        var __VLS_6 = ({ toggle: {} },
            { onToggle: function () {
                    var _a = [];
                    ...
                    // @ts-ignore
                    [headerBaseLabel, headerBaseLabel, formatTotalsLine, formatTotalsLine, formatTotalsLine, headerTotals, headerTotals, onAdd, onAdd, items, filteredItems, grouped, categoryClass, categoryTotals, categoryTotals, baseTotalLabel, categoryPercent, shouldShowBaseTotal, categoryBaseValue, isLiabilitiesList, expandedCats, toggleCategory,];
                } });
        if (__VLS_ctx.shouldShowGroupDetails(g)) {
            __VLS_asFunctionalElement1(__VLS_intrinsics.div, __VLS_intrinsics.div)(__assign({ class: (g.hasSubgroups ? 'subcat-list' : '') }));
            for (var _d = 0, _e = __VLS_vFor((g.subgroups)); _d < _e.length; _d++) {
                var _f = _e[_d], sg = _f[0], sgIndex = _f[1];
                __VLS_asFunctionalElement1(__VLS_intrinsics.div, __VLS_intrinsics.div)(__assign({ key: (__VLS_ctx.subgroupKey(g, sg, sgIndex)) }, { class: (g.hasSubgroups ? 'nw-subcat-block' : '') }));
                if (g.hasSubgroups) {
                    ...
                    var __VLS_8 = __VLS_asFunctionalComponent1(__VLS_7, new __VLS_7({
                        ...
                        percent: (__VLS_ctx.subcategoryPercent(g.category, sg.subcategory, sg.items, g.items)),
                        showBaseTotal: (__VLS_ctx.shouldShowBaseTotal(__VLS_ctx.categoryTotals(sg.items), __VLS_ctx.subcategoryBaseValue(g.category, sg.subcategory, sg.items))),
                    }));
                ...
                var _loop_2 = function (it) {
                    ...
                    if (__VLS_ctx.editingId !== it.id) {
                        ...
                        // @ts-ignore
                        var __VLS_13 = __VLS_asFunctionalComponent1(__VLS_12, new __VLS_12(__assign(__assign(__assign({ 'onEdit': {} }, { 'onArchive': {} }), { 'onDelete': {} }), { item: (it), formattedAmount: (__VLS_ctx.formatAmountWithUnit(__VLS_ctx.displayAmount(it), it.currency)), isLiabilitiesList: (__VLS_ctx.isLiabilitiesList), financedAssetName: (__VLS_ctx.financedAssetName(it.financed_asset_ref)), ownershipLabel: (__VLS_ctx.ownershipShortLabel(it.ownership_ref)), sharePercent: (it._sharePercent) })));
                        var __VLS_14 = __VLS_13.apply(void 0, __spreadArray([__assign(__assign(__assign({ 'onEdit': {} }, { 'onArchive': {} }), { 'onDelete': {} }), { item: (it), formattedAmount: (__VLS_ctx.formatAmountWithUnit(__VLS_ctx.displayAmount(it), it.currency)), isLiabilitiesList: (__VLS_ctx.isLiabilitiesList), financedAssetName: (__VLS_ctx.financedAssetName(it.financed_asset_ref)), ownershipLabel: (__VLS_ctx.ownershipShortLabel(it.ownership_ref)), sharePercent: (it._sharePercent) })], __VLS_functionalComponentArgsRest(__VLS_13), false));
                        var __VLS_17 = void 0;
                        var __VLS_18 = ({ edit: {} },
                            { onEdit: function () {
                                    var _a = [];
                                    ...
                                    if (!(__VLS_ctx.editingId !== it.id))
                                        return;
                                    __VLS_ctx.onEdit ? __VLS_ctx.onEdit(__VLS_ctx.editTarget(it)) : __VLS_ctx.startEdit(__VLS_ctx.editTarget(it));
                                    // @ts-ignore
                                    [formatTotalsLine, categoryTotals, categoryTotals, shouldShowBaseTotal, isLiabilitiesList, shouldShowGroupDetails, subgroupKey, subcategoryBaseLabel, subcategoryPercent, subcategoryBaseValue, editingId, formatAmountWithUnit, displayAmount, financedAssetName, ownershipShortLabel, onEdit, onEdit, editTarget, editTarget, startEdit,];
                                } });
                        var __VLS_19 = ({ archive: {} },
                            { onArchive: function () {
                                    var _a = [];
                                    for (var _i = 0; _i < arguments.length; _i++) {
                                        _a[_i] = arguments[_i];
                        ...
                        // @ts-ignore
                        var __VLS_22 = __VLS_asFunctionalComponent1(__VLS_21, new __VLS_21(__assign(__assign({ 'onSave': {} }, { 'onCancel': {} }), { draft: (__VLS_ctx.draft), categories: (__VLS_ctx.categories), subcategories: (props.subcategories), currencies: (__VLS_ctx.currencies), ownershipOptions: (__VLS_ctx.ownershipOptions), showFinancedAsset: (__VLS_ctx.isLiabilitiesList), financedAssetOptions: (__VLS_ctx.financedAssetOptions), amountHint: (__VLS_ctx.amountHint), amountError: (__VLS_ctx.amountError) })));
                        var __VLS_23 = __VLS_22.apply(void 0, __spreadArray([__assign(__assign({ 'onSave': {} }, { 'onCancel': {} }), { draft: (__VLS_ctx.draft), categories: (__VLS_ctx.categories), subcategories: (props.subcategories), currencies: (__VLS_ctx.currencies), ownershipOptions: (__VLS_ctx.ownershipOptions), showFinancedAsset: (__VLS_ctx.isLiabilitiesList), financedAssetOptions: (__VLS_ctx.financedAssetOptions), amountHint: (__VLS_ctx.amountHint), amountError: (__VLS_ctx.amountError) })], __VLS_functionalComponentArgsRest(__VLS_22), false));
                        var __VLS_26 = void 0;
                        var __VLS_27 = ({ save: {} },
                            { onSave: function () {
                                    var _a = [];
                                    ...
                                    // @ts-ignore
                                    [isLiabilitiesList, draft, categories, currencies, ownershipOptions, financedAssetOptions, amountHint, amountError, saveEdit,];
                                } });
                        var __VLS_28 = ({ cancel: {} },
