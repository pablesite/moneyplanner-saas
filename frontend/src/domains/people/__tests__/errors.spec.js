...
(0, vitest_1.describe)('people error mapper', function () {
  ...
  (0, vitest_1.it)('uses detail/first field or axios message as fallback', function () {
    var spy = vitest_1.vi.spyOn(axios_1.default, 'isAxiosError').mockReturnValue(true);
    (0, vitest_1.expect)(
      (0, errors_1.toPeopleErrorMessage)({ response: { data: { detail: 'detail-msg' } } }),
    ).toBe('detail-msg');
    (0, vitest_1.expect)(
      (0, errors_1.toPeopleErrorMessage)({ response: { data: { field: ['first-error'] } } }),
    ).toBe('field: first-error');
    (0, vitest_1.expect)(
      (0, errors_1.toPeopleErrorMessage)({ response: { data: null }, message: 'axios-msg' }),
    ).toBe('axios-msg');
    spy.mockRestore();
  });
});
