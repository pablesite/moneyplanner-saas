...
(0, vitest_1.describe)('expense taxonomy (core)', function () {
    (0, vitest_1.it)('defines subcategories for every category', function () {
        var categories = expenseTaxonomy_1.expenseCategories.map(function (c) { return c.value; });
        var _loop_1 = function (category) {
            var rows = expenseTaxonomy_1.expenseSubcategories.filter(function (sub) { return sub.category === category; });
    ...
    (0, vitest_1.it)('includes at least one fallback subcategory per category', function () {
        ...
        var _loop_2 = function (category) {
            var rows = expenseTaxonomy_1.expenseSubcategories.filter(function (sub) { return sub.category === category; });
            var hasFallback = rows.some(function (sub) { return sub.value.startsWith('other') || sub.value === 'other'; });
