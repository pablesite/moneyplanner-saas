...
function formatImportYearSummary(entries, label) {
  var _a, _b;
  if (!entries.length) return ''.concat(label, ': 0');
  var totals = new Map();
  ...
  var segments = __spreadArray([], totals.entries(), true)
    .sort(function (a, b) {
      return a[0] - b[0];
    })
    .map(function (_a) {
...
function buildImportPreviewMessageWithVersion(bundle, mode, currentAppVersion) {
  ...
  var lines = [
    ...
    '- Version exportada: '.concat(
      exportedVersion !== null && exportedVersion !== void 0
        ? exportedVersion
        : 'legacy/sin version',
    ),
    '- Version actual: '.concat(
      currentAppVersion !== null && currentAppVersion !== void 0
        ? currentAppVersion
        : 'no disponible',
    ),
    ...
    '- Configuracion base_currency: '.concat(
      (_c = (_b = bundle.settings) === null || _b === void 0 ? void 0 : _b.base_currency) !==
        null && _c !== void 0
        ? _c
        : 'sin incluir',
    ),
    hasPremium
      ? '- Miembros: '
          .concat(
            (_e =
              (_d = bundle.premium) === null || _d === void 0
                ? void 0
                : _d.family_members.length) !== null && _e !== void 0
              ? _e
              : 0,
            ' | Titularidades: ',
          )
          .concat(
            (_g =
              (_f = bundle.premium) === null || _f === void 0 ? void 0 : _f.ownerships.length) !==
              null && _g !== void 0
              ? _g
              : 0,
            ' | Enlaces: ',
          ...
          .concat(
            (_j =
              (_h = bundle.premium) === null || _h === void 0
                ? void 0
                : _h.ownership_links.length) !== null && _j !== void 0
              ? _j
              : 0,
          )
...
function toPortableLiabilityRecord(raw) {
  ...
  var financedAssetRef =
    (_b = (_a = raw.financed_asset_ref) !== null && _a !== void 0 ? _a : raw.financed_asset_id) !==
      null && _b !== void 0
      ? _b
      : null;
  return {
...
function parsePortableDataBundle(raw) {
  ...
  if (
    (parsed === null || parsed === void 0 ? void 0 : parsed.schema_version) !== 1 ||
    !parsed.data
  ) {
    throw new Error('Formato de archivo no compatible.');
  }
  var _c = parsed.data,
    annual_income = _c.annual_income,
    annual_expense = _c.annual_expense,
    assets = _c.assets,
    liabilities = _c.liabilities,
    snapshots = _c.snapshots;
  if (
  ...
  if (parsed.premium != null) {
    ...
    premium = {
      family_members: premiumRaw.family_members.map(function (row) {
        return toPortableFamilyMemberRecord(row);
      }),
      ownerships: premiumRaw.ownerships.map(function (row) {
        return toPortableOwnershipRecord(row);
      }),
      ownership_links: premiumRaw.ownership_links.map(function (row) {
        return toPortableOwnershipLinkRecord(row);
      }),
    };
  }
  return {
      ...
      parsed.settings && typeof parsed.settings === 'object'
        ? {
            base_currency: String(
              (_b = parsed.settings.base_currency) !== null && _b !== void 0 ? _b : '',
            ),
          }
        : undefined,
    data: {
      annual_income: annual_income.map(function (row) {
        return toPortableAnnualIncomeRecord(row);
      }),
      annual_expense: annual_expense.map(function (row) {
        return toPortableAnnualExpenseRecord(row);
      }),
      assets: assets.map(function (row) {
        return toPortableAssetRecord(row);
      }),
      liabilities: liabilities.map(function (row) {
        return toPortableLiabilityRecord(row);
      }),
