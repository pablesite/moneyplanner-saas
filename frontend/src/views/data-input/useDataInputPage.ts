...
export function useDataInputPage() {
  ...
  const filteredExpenseCashflowRoleOptions = computed<SelectOption[]>(() => {
    const allowed = new Set(
      allowedExpenseCashflowRolesForTimeProfile(annualExpenseForm.timeProfile),
    );
  ...
  const visibleAssetsByOwner = computed(() =>
    visibleAssets.value.filter(
      (item) =>
        allocatedFractionForNetWorthOwner(item.ownership_ref, assetOwnershipFilter.value) > 0,
    ),
  );
  ...
  function openLiabilityModal(): void {
  ...
  function summarizeGeneratedLiabilityExpenseChanges(
    ...
    const toYearTotals = (rows: AnnualExpenseEntry[]) => {
      ...
      for (const row of rows) {
        totals.set(
          row.fiscalYear,
          (totals.get(row.fiscalYear) ?? 0) + Number(row.amountAnnual ?? 0),
        );
      }
      return totals;
    };
  ...
  async function submitLiabilityWithExpenseReview(
    ...
    try {
      ...
      const systemGeneratedEntries = generatedEntries
        .filter(
          (entry) => entry.isSystemGenerated && entry.sourceLiabilityId === createdLiability.id,
        )
    ...
    if (
      ...
      try {
        ...
        if (
          ...
          bulkEditingGeneratedLiabilityId.value
        ) {
