...
(0, vitest_1.describe)('core api error helper', function () {
  ...
  (0, vitest_1.it)('prioritizes validation details from API envelope', function () {
    ...
    var spy = vitest_1.vi.spyOn(axios_1.default, 'isAxiosError').mockReturnValue(true);
    (0, vitest_1.expect)((0, errors_1.toApiErrorMessage)(err)).toBe(
      'market_value_override_date: Requerida si se informa valor de mercado manual.',
    );
    spy.mockRestore();
  });
  (0, vitest_1.it)(
    'falls back to envelope message, validation payload, string payload, error message and default',
    function () {
      var spy = vitest_1.vi.spyOn(axios_1.default, 'isAxiosError').mockReturnValue(true);
      (0, vitest_1.expect)(
        (0, errors_1.toApiErrorMessage)({
      ...
      (0, vitest_1.expect)(
        (0, errors_1.toApiErrorMessage)({ response: { data: 'plain' }, message: 'x' }),
      ).toBe('plain');
      (0, vitest_1.expect)((0, errors_1.toApiErrorMessage)({ message: 'fallback' })).toBe(
        'fallback',
      );
